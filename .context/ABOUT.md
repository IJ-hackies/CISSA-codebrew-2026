...
> **PIVOT IN PROGRESS.** This project is actively pivoting to align with the **Stellar Stories** hackathon track (data and cultural preservation). The full pivot plan, including what changes where and how big each change is, lives in **[REVAMP.md](REVAMP.md)**. All new work should reference REVAMP.md for direction. The description below reflects the original architecture — the pipeline and backend are largely preserved, but the frontend is being fully rewritten and the framing is shifting from "AI study tool" to "AI-powered cultural preservation platform with 3D spatial exploration."

## What Is This?

Scholar System is an AI-powered cultural preservation platform that transforms any body of information — historical texts, personal photo archives, text messages, event timelines, oral histories — into a navigable 3D galaxy. The AI pipeline processes everything, finds connections between pieces of knowledge, and builds a cosmos out of your data that you can fly through, drill into, and experience through rich interactive content.
...
**Stage 2 — Detail (parallel per-concept sub-sessions).** Each concept gets its own isolated proxy sub-session (`galaxyId--d--conceptId`) with a focused Claude Code agent (Sonnet 4.6) that creates exactly ONE detail file. Sub-sessions run in parallel, bounded by the proxy worker pool (`maxConcurrency: 10`). Each agent receives only its concept's source units + skeleton context (brief, kind, sourceRefs, neighbor list for wikilinks). Output: one `stage2-detail/<conceptId>.md` file with 200-300 word body + `# Derivatives` section (verbatim quotes) + `# Relations` section (typed edges). A separate auxiliary sub-session handles `_structure.md` and `_etc.md` for non-concept source units. After all sub-sessions complete, files are merged and pushed back to the main workspace (preserving the folder-to-folder chain), then compiled. Missing concepts get up to 2 retry rounds with fresh sub-sessions. The `compileDetail()` step parses `# Derivatives` sections via `extractDerivatives()` in `frontmatter.ts`. Implementation: `pipeline/detail.ts` + `prompts/detail.ts` (`buildConceptDetailPrompt`, `buildAuxiliaryDetailPrompt`). The fan-out infrastructure (`fanOutSubSessions`, `mergeSubSessionFiles`) lives in `lib/proxy-client.ts`.
...
**Stage 5 — Visuals (parallel per-body sub-sessions).** Haiku 4.5. Each knowledge-bearing body gets its own isolated proxy sub-session (`galaxyId--v--bodyId`) with a focused Claude Code agent that creates exactly ONE visual file. **Two-wave execution:** wave 1 themes systems/galaxy bodies (to extract palettes), wave 2 themes planets/moons/asteroids with parent system palette hints for coherence. All sub-sessions within a wave run in parallel. After collection, files are merged and pushed back to the main workspace. Append-only: existing entries are frozen. Decorative bodies (star, nebula, comet, black-hole, dust-cloud, asteroid-belt) are code-themed from `narrative.canon.aesthetic` via `generateDecorativeVisual()`, zero Claude calls. Moon and asteroid visuals carry two additional fields: **`biome`** (one of 10 environment types — all concepts within one subtopic share a biome, pre-computed by `assignBiomesPerSubtopic`) and **`character`** (which NPC appears in scenes — pre-computed by `assignCharactersPerConcept` from concept kind). Character mapping: definition/principle/paradigm → sage, formula/process/framework → engineer, example/distinction/property → archivist, fact/trade-off → trickster; warrior and echo are rare, reserved for heavy-tier concepts. Implementation: `pipeline/visuals.ts` + `prompts/worldgen/visuals.ts` (`buildSingleBodyVisualPrompt`). Fan-out via `fanOutSubSessions` in `lib/proxy-client.ts`.
...
**AI.** All Claude calls go through Claude Code, never the metered Messages API. Default model is Sonnet 4.6 across Stages 1–3. **Stage 5 uses Haiku 4.5** — visual param generation (palettes, terrain enums, mood strings) doesn't need Sonnet-level reasoning. Opus 4.6 appears only in Stage 6 scene generation for `modelTier: heavy` concepts; Haiku 4.5 handles Stage 6 `light` concepts. Stages 2 and 5 use **parallel sub-sessions** (one Claude Code process per concept/body) instead of one large session per topic — the proxy worker pool (`maxConcurrency: 10`) throttles parallel Claude Code processes. Files live on disk inside the proxy's workspace — Claude Code reads and writes them natively, which removes prompt-ballooning for file-heavy stages and makes runs inspectable. `pdf-parse` and equivalent extractors run in Stage 0 on the API server side.
