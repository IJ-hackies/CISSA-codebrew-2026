...
**Current implementation (2026-04-10):** Stages 0, 1, 2, 4 are wired. The HTTP route `POST /api/galaxy/create` runs the **fast path** (ingest → structure → layout) synchronously, persists, responds, then fires Stage 2 (detail) as a fire-and-forget background job that re-persists the galaxy when it finishes. Stage 2 itself parallelises across topics via `Promise.allSettled` (one Claude call per topic + one for loose concepts). The CLI harness `server/src/scripts/test-pipeline.ts` deliberately awaits Stage 2 synchronously so it can print coverage. Stages 3, 5, 6 not yet implemented — their scopes stay at empty defaults and `pipeline[stage].status` remains `"pending"`. Blobs are still schema-valid and renderable from just `knowledge` + `relationships` + `spatial`, so the frontend never has to wait for detail. Frontend is NOT notified when background detail lands — deliberate silent-population strategy until a UI surface actually needs detail state (see the execution-model header in `server/src/pipeline/runner.ts` for the polling/SSE upgrade paths).
